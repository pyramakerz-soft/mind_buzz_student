import 'package:flutter/material.dart';
import 'package:flutter_bloc/flutter_bloc.dart';
import 'package:flutter_screenutil/flutter_screenutil.dart';
import 'package:flutter_svg/svg.dart';

import '../../../../../../core/phonetics/phonetics_color.dart';
import '../../../../domain/entities/game_letters_model.dart';
import '../manager/bingo_cubit.dart';
import '../widget/item_card_widget.dart';

class BingoGameScreen extends StatelessWidget {
  @override
  Widget build(BuildContext context) {
    return Container(
        alignment: Alignment.center,
        // height: MediaQuery.of(context).size.height - (70.h),

        margin: EdgeInsets.only(right: 15.w),
        width: MediaQuery.of(context).size.width - (100.w),
        height: MediaQuery.of(context).size.height - 150.h,
        padding: EdgeInsets.all(1),
        decoration: BoxDecoration(
            color: Colors.white,
            borderRadius: BorderRadius.circular(15),
            border:
                Border.all(color: AppColorPhonetics.boarderColor, width: 5)),
        child: BlocConsumer<BingoCubit, BingoInitial>(
            listener: (context, state) {},
            builder: (context, gameState) {
              return Center(
                  child:Wrap(
                    children: List.generate((gameState.cardsLetters)?.length??0, (rowIndex) => (gameState.cardsLetters)?[rowIndex].id == null
                        ? SizedBox(
                      width: (MediaQuery.of(context)
                          .size
                          .width -
                          (106.w))/3,
                      // height: 75,
                    )
                        : (gameState.cardsLetters)?[rowIndex].hide == true
                        ? SizedBox(
                      width: 75,
                      height: 75,
                    )
                        : ItemCardWidget(
                      onTap: () {
                        // screenCloseTime =
                        //     DateTime.now();
                        // log('#######################');
                        // log(item.value.letter
                        //     .toString());
                        // log(randomVisibleLetter
                        //         ?.letter
                        //         .toString() ??
                        //     '');
                        //
                        // log('#######################');
                        // if (ttsState !=
                        //     TtsState.playing) {
                        //   context
                        //       .read<Game2Provider>()
                        //       .addTheStateOfCurrentAnswer(
                        //           context: context,
                        //           userAnswer: item
                        //               .value.letter
                        //               .toString(),
                        //           correctAnswer:
                        //               randomVisibleLetter
                        //                       ?.letter
                        //                       .toString() ??
                        //                   '');
                        //   if (item.value.letter
                        //           .toString() ==
                        //       randomVisibleLetter
                        //           ?.letter
                        //           .toString()) {
                        //     context
                        //         .read<Game2Provider>()
                        //         .correctAnswer(
                        //             (startIndex +
                        //                 item.key),
                        //             context,
                        //             screenCloseTime);
                        //   } else {
                        //     int countOfRepeatQuestion =
                        //         context
                        //             .read<
                        //                 StartGameProvider>()
                        //             .startAnimation();
                        //     if (countOfRepeatQuestion ==
                        //         0) {
                        //       final p = context.read<
                        //           Game2Provider>();
                        //
                        //       showDialog(
                        //         barrierDismissible:
                        //             false,
                        //         context: context,
                        //         builder: (BuildContext
                        //                 context0) =>
                        //             ChangeNotifierProvider<
                        //                     StartGameProvider>.value(
                        //                 value: p,
                        //                 child:
                        //                     RepeatQuestionPopUp(
                        //                   onTap: () {
                        //                     context
                        //                         .read<
                        //                             Game2Provider>()
                        //                         .getDataOfGame(
                        //                             context);
                        //                     Navigator.of(
                        //                             context)
                        //                         .pop();
                        //                   },
                        //                 )),
                        //       );
                        //     }
                        //   }
                        // }
                      },
                      body: (gameState.cardsLetters)?[rowIndex].letter ?? '',
                      maxHeight:
                      MediaQuery.of(context)
                          .size
                          .height -
                          162.h,
                      maxWidth: MediaQuery.of(context)
                          .size
                          .width -
                          (107.w),
                    )),
                  )

            );}
    )
    );
  }
}
