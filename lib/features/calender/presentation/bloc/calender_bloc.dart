...
class CalenderBloc extends Bloc<CalenderEvent, CalenderState> {
  ...
  CalenderBloc({required this.programUserUseCases})
      : super(GetCalenderInitial()) {
  ...
  List<TestModel> get filteredList => tests
      .where((element) =>
          Utils.parseStringToDate(element.createdAt!) ==
          Utils.formatDate(focusedDay))
      .toList();
  ...
  getAssignments(emit) async {
    ...
    try {
      ...
      res.fold((l) {
        ...
        tests = data.tests ?? [];
        emit(GetCalenderCompleteInitial(
            tests: filteredList, currentDate: focusedDay));
      });
    } catch (e) {
      log('getUserData $e ');
  ...
  swapCalender(emit, Direction direction) {
    ...
    if (direction == Direction.left) {
      focusedDay = focusedDay.month < 12
